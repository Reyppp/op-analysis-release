...
- 展示光谱波形、Delta CW、EXP 点位及内圈、监控点、外圈分区结果；匹配度波形以监控点中心波长对齐，并按内圈、监控点、外圈每层 0.1 dB 清晰错层显示。
- 结合薄片数量、外观、规格和测试指标，输出预估划切颗粒数、预估测试合格数、预估出库数及预测区间。
- 自动区分外观等级、量化缺陷和未量化影响；无法识别的原文片段会完整保留并以2%默认影响供人工调整。
- 生成可放大查看的分析报告；报告详情图使用更宽的波形区域，光谱截图与测试页当前的 `0～−1 dB` / `Zoom` 模式保持一致。
...
2. 在“测试数据”核对规格、九项指标、光谱波形和 Delta CW；需要三圈对比时选择 EXP 列表第一行“匹配度”。三圈原始数据不变，错层仅用于分辨波形形态；可直接点击“截图”复制完整匹配度卡片。
3. 在“炉次概览”确认薄片数量与外观描述；核对自动识别的未量化影响，必要时手动修改后再查看三阶段预测和风险提示。
