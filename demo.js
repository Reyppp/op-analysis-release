(() => {
  ...
  const guideFeatures = {
    ...
    "spec-manager": ["规格判断", "规格管理", "维护带宽、Ripple、隔离度等规格目标。", "新增产品规格或工程要求发生变化时使用。", "打开规格管理，填写名称与各指标范围后保存。", "不要用临时目标覆盖正式规格，必要时新建规格。", "data"],
    ...
    "lot-id": ["基础信息", "炉号", "显示程序从文件中识别的当前炉次编号。", "核对导入内容是否属于目标炉次时使用。", "将炉号与原始文件夹、生产记录进行比对。", "公开教程使用 DEMO 编号，不代表真实生产数据。", "overview"],
    "slice-count": ["基础信息", "薄片数量", "显示自动识别的薄片数，并支持桌面端手动修改。", "文件缺失或实际片数与识别结果不一致时使用。", "核对自动值；需要时手动输入，恢复默认可重新采用识别值。", "理论满面颗粒数会随片数直接变化。", "overview"],
    "appearance": ["必要输入", "外观描述", "将 60-40、缺陷和未量化外观风险纳入可用面积判断。", "每次产出分析都必须填写。", "按生产记录输入外观等级、占比、发蒙、分散大亮点等描述。", "使用事实描述和明确占比；无法量化的影响仍可在桌面端手动调整。", "overview"],
    ...
    "calculation-detail": ["结果解释", "计算详情", "展开中间率、约束上限、烘烤及加工风险。", "中心值受硬上限或风险影响，需要追溯原因时使用。", "展开详情，按模型基准、指标约束、烘烤损失和最终结果顺序阅读。", "合格率展示口径不同，不应重复相乘。", "overview"],
    ...
    "copy-report": ["报告输出", "复制报告", "将报告表格复制到剪贴板，便于粘贴到 Excel。", "报告内容确认无误后使用。", "点击复制报告，再到 Excel 中直接粘贴。", "表格尺寸设置保存在当前电脑，页面缩放不改变输出尺寸。", "report"],
    "report-table": ["报告校对", "报告预览", "显示即将复制的外观、均匀性、指标和产出内容。", "复制前进行最后一次人工核对时使用。", "检查每个单元格；桌面端可拖动列宽和行高。", "公开页面展示的日期、炉号和数量均为虚构数据。", "report"]
  };
  ...
  document.getElementById("guideSearch").addEventListener("submit", event => {
    ...
    if (query && match) showGuideFeature(match[0]);
      ...
      document.getElementById("guideDetailCopy").textContent = query ? "可尝试搜索导入、规格、Delta CW、风险、模型、产出或报告。" : "输入按钮名称或分析任务即可快速定位。";
    }
  });
  ...
  document.getElementById("releaseVersion").textContent = release.version || "0.2.0-beta.2";
  if (release.public) {
