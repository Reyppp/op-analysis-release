(() => {
  ...
  const guideFeatures = {
    ...
    "copy-report": ["报告输出", "复制报告", "将当前炉次复制为一行五列：日期/产品、炉号/PN、预估产出/薄片、合成图、分析内容。", "报告内容确认无误、准备粘贴到既有 Excel 模板时使用。", "先在 Excel 选中任意目标起始单元格，再回到软件点击复制报告并粘贴；内容从所选单元格开始占用一行五列。", "软件不限定 A2，也不主动修改模板行高和列宽。D 列是一张高清合成图，文字使用微软雅黑 10 号。", "report"],
    "report-images": ["报告校对", "报告图片", "依次展示 Delta CW、内圈、监控点、外圈和匹配度五张图。", "需要在复制前检查曲线、指标截图或匹配度细节时使用。", "在桌面端点击任意报告图片即可放大，按 Esc、点击遮罩或关闭按钮退出。", "软件预览保留五张独立图片；复制到 Excel 时会合成为一张高清图片，避免重叠和跨行。", "report"],
  ...
  document.getElementById("releaseVersion").textContent = release.version || "0.3.0-beta.6";
  if (release.public) {
