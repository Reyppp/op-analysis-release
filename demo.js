(() => {
  ...
  const guideFeatures = {
    ...
    "report-images": ["报告校对", "报告图片", "依次展示 Delta CW、内圈、监控点、外圈和匹配度五张图；光谱截图跟随测试页当前的 0～−1 dB 或 Zoom 模式。", "需要在复制前检查曲线、坐标范围、指标截图或匹配度细节时使用。", "先在测试页选好波形模式，再生成或刷新报告；在桌面端点击任意报告图片可放大，按 Esc、点击遮罩或关闭按钮退出。", "软件预览保留五张独立图片；复制到 Excel 时会合成为一张高清图片，避免重叠和跨行。", "report"],
  ...
  document.getElementById("releaseVersion").textContent = release.version || "0.3.0-beta.7";
  if (release.public) {
